...
// --- QuadraticProbingHashTable Implementation ---
QuadraticProbingHashTable::QuadraticProbingHashTable(int capacity)
{
  tablesize = capacity;
  size = 0;
  technique = QUADRATIC; // Always quadratic
...
int QuadraticProbingHashTable::IsEmpty() { return size == 0 ? 1 : 0; }
int QuadraticProbingHashTable::IsFull() { return tablesize == size ? 1 : 0; }

int QuadraticProbingHashTable::Display()
{
 if (IsEmpty()) return 1;
...
int QuadraticProbingHashTable::MakeHashingEmpty()
{
 if (IsEmpty()) return 1;
 for (int i = 0; i < tablesize; ++i) table[i] = EmployeeData();
 size = 0;
...
int QuadraticProbingHashTable::Hash(int k) { return (k % tablesize); }

// Only Quadratic Probing calculation
int QuadraticProbingHashTable::Probe(int key, int i)
{
    // H(k, i) = (H(k) + i^2) mod TableSize
    int bucketID = (Hash(key) + (i * i)) % tablesize;
    if (bucketID < 0) bucketID += tablesize;
    return bucketID;
}

int QuadraticProbingHashTable::Insert(EmployeeData emp)
{
  if (IsFull()) return 1;
...
int QuadraticProbingHashTable::Delete(int k)
{
  if (IsEmpty()) return 1;
  int i, bucketID;
  for(i = 0; i < tablesize; i++)
...
int QuadraticProbingHashTable::Search(int k)
{
  if (IsEmpty()) return 1;
  int i, bucketID;
  for(i = 0; i < tablesize; i++)
